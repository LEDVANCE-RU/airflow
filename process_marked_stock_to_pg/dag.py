...
from process_marked_stock_to_pg.libs.transform import transform_si_data
from process_marked_stock_to_pg.libs.upload import PgSiHook

SI_DIR_NAME = 'marked_stock'

with DAG(
    dag_id="process_marked_stock_to_pg",
    start_date=datetime(2025, 5, 1, tzinfo=TZ_MSK),
    schedule='55 4 * * 1-5',
    catchup=False,
    tags=['1c', 'marked_stock', 'postgresql'],
) as dag:
    @task
    def download_task() -> str:
        ...
        required_files = ["marked_stock_ic"]
